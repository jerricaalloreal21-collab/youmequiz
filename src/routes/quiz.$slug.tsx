...
function QuestionnairePage(){
  ...
  const [index,setIndex]=useState(0); const [done,setDone]=useState(false);
  const result=useMemo(()=>done?scoreQuestionnaire(quiz,answers):null,[done,quiz,answers]);
  ...
  return <AppShell><section className="rise-in pt-2">
    ...
    <div className="mt-2 h-2 overflow-hidden rounded-full bg-card"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${Math.max(pct,4)}%`}}/></div>
    <p className="mt-6 text-xs font-semibold uppercase tracking-wide text-accent">{quiz.emoji} {quiz.title}</p>
    ...
    <div className="mt-6 space-y-3">{question.choices.map((c,i)=><button key={`${c.category}-${i}`} onClick={()=>choose(c.category)} className="card-soft tap-scale w-full px-4 py-4 text-left text-[15px] leading-snug hover:border-primary">{c.text}</button>)}</div>
    <button disabled={index===0} onClick={()=>setIndex(i=>Math.max(0,i-1))} className="mt-6 inline-flex items-center gap-1 text-sm text-muted-foreground disabled:opacity-40"><ArrowLeft className="size-4"/> Back</button>
  </section></AppShell>
}

function Result({quiz,result,restart}:{quiz:Questionnaire;result:ReturnType<typeof scoreQuestionnaire>;restart:()=>void}){
 ...
 return <AppShell><section className="rise-in pt-2">
  ...
  <div className="mt-8 flex flex-col gap-3"><Button variant="hero" size="xl" onClick={restart}><RotateCcw/> Take it again</Button><Button asChild variant="soft" size="xl"><Link to="/">Choose another questionnaire</Link></Button></div>
  <p className="mt-8 rounded-2xl border border-border bg-card/60 p-4 text-xs leading-relaxed text-muted-foreground">YouMeQuiz is for self-reflection and entertainment. Results describe only your answers and are not a diagnosis, clinical assessment, or compatibility guarantee.</p>
 </section></AppShell>
}
function Block({title,children}:{title:string;children:React.ReactNode}){return <section className="mt-8"><h2 className="text-lg">{title}</h2><div className="mt-3">{children}</div></section>}
